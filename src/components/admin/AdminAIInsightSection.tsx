...
const statusClassByKey: Record<string, string> = {
  ...
  pending_review: "bg-sky-50 text-sky-700 ring-1 ring-sky-100",
  reviewed: "bg-indigo-50 text-indigo-700 ring-1 ring-indigo-100",
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {activeAnalysis && (
          <div className="flex flex-wrap gap-2">
            <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusClassByKey[activeAnalysis.status]}`}>
              {labelize(activeAnalysis.status)}
            </span>
            <span
              className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${
                statusClassByKey[activeAnalysis.review_state] ?? statusClassByKey.archived
              }`}
            >
