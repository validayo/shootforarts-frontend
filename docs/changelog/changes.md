...
### AI admin workflow rollout

- Added the Phase 1 AI admin dashboard flow for inquiry analysis and review inside the existing admin UI.
- Added dashboard AI summary/list/detail integration rather than a separate AI product surface.
- Added draft review actions for editing, approving, copying, and manual send confirmation.
- Kept the active send path human-controlled:
  - copy draft
  - send manually outside the app
  - mark as sent inside admin
- Why: introduce useful AI assistance without forcing live provider send or redesigning the admin workflow.

### AI admin UX refinement

- Reduced row-level AI badge noise so older or pre-AI inquiries do not misleadingly look like they still need AI work.
- Reworked inquiry detail behavior across desktop, tablet, and mobile so the AI panel remains usable inside the current admin flow.
- Tightened the AI detail surface and deprioritized noisy status clutter.
- Why: make the AI layer feel assistive instead of bolted on, and keep smaller-screen admin use practical.

### AI planning docs refresh

- Updated the AI implementation plan, schema/API doc, and tech ops runbook to reflect the current shipped workflow and the next likely roadmap.
- Captured the next recommended direction as:
  - context-aware rewrite/regenerate
  - later inquiry assistant support
  - later operational automation like invoice/contract generation
- Why: keep future work scoped to real admin workflow needs instead of drifting into a generic chatbot too early.

## 2026-04-21

### Contracts admin rollout + backend alignment

- Added the Contracts admin surface under `/sfaadmin/contracts` with:
  - draft list
  - create flow
  - manifest-driven editor
  - read-only document preview
  - save flow
  - archive-backed delete flow using the dedicated backend delete endpoint
- Aligned the frontend with the updated contracts backend:
  - removed old noisy manifest assumptions
  - used backend `uiGroup: "advanced"` metadata for advanced fields
  - switched delete from archive-via-save to `admin-contracts-delete`
- Why: ship the first usable contract drafting workflow while keeping backend ownership of manifest/schema behavior.

### Contract preview + export refinement

- Reworked the contract preview into a more document-like workspace with fixed sizing, separate internal scrolling, and paginated preview behavior.
- Reworked the contract HTML/document builder so preview and PDF/save-as-PDF stay closer in structure and formatting.
- Tightened contract output around:
  - event/portrait wording
  - signature formatting
  - delivery window behavior
  - section inclusion handling
  - removal of leaked special-notes text
- Why: make the contract workflow feel like a real editable document instead of a raw admin form dump.

### Feature-first frontend structure refactor

- Reorganized the frontend into a feature-first structure under `src/features/`.
- Moved admin modules into:
  - `src/features/admin/assistant/`
  - `src/features/admin/auth/`
  - `src/features/admin/contracts/`
  - `src/features/admin/dashboard/`
  - `src/features/admin/data/`
  - `src/features/admin/gallery/`
  - `src/features/admin/shared/`
  - `src/features/admin/upload/`
- Moved public modules into:
  - `src/features/about/`
...
- Kept `src/pages/` as a thinner route-entry layer and reduced `src/components/` to truly shared layout/routing/seo/ui code.
- Why: make navigation and ownership clearer as the app grows beyond a small shared-components structure.

### Docs + structure documentation refresh
