...
const AdminGalleryManager: React.FC = () => {
  ...
  const handleToggleTop = useCallback(
    async (photo: Photo) => {
      ...
      try {
        ...
        logAdminAction("gallery.top_toggled", { photoId: photo.id, isTop: makeTop });
      } catch (err) {
        const text = err instanceof Error ? err.message : "Failed to update Top status.";
        logAdminError("gallery.top_toggle_failed", { photoId: photo.id, message: text });
        setError(text);
      } finally {
        setBusyPhotoId(null);
      }
  ...
  const handleSeasonChange = useCallback(
    async (photo: Photo, season: string) => {
      ...
      try {
        ...
        logAdminAction("gallery.season_changed", { photoId: photo.id, season: nextSeason });
      } catch (err) {
        const text = err instanceof Error ? err.message : "Failed to update season tag.";
        logAdminError("gallery.season_change_failed", { photoId: photo.id, message: text });
        setError(text);
      } finally {
        setBusyPhotoId(null);
      }
  ...
  const handleDeletePhoto = useCallback(
    async (photo: Photo) => {
      ...
      try {
        ...
        logAdminAction("gallery.photo_deleted", { photoId: photo.id });
      } catch (err) {
        const text = err instanceof Error ? err.message : "Failed to delete photo.";
        logAdminError("gallery.photo_delete_failed", { photoId: photo.id, message: text });
        setError(text);
      } finally {
        setBusyPhotoId(null);
      }
  ...
  const handleSaveTopOrder = useCallback(async () => {
    ...
    try {
      ...
      logAdminAction("gallery.top_order_saved", { count: updates.length });
    } catch (err) {
      const text = err instanceof Error ? err.message : "Failed to save Top order.";
      logAdminError("gallery.top_order_save_failed", { message: text });
      setError(text);
    } finally {
      setSavingTop(false);
    }
  ...
  const handleSaveSeasonOrder = useCallback(async () => {
    ...
    try {
      ...
      logAdminAction("gallery.season_order_saved", { season: currentSeason, count: updates.length });
    } catch (err) {
      const text = err instanceof Error ? err.message : "Failed to save season order.";
      logAdminError("gallery.season_order_save_failed", { season: currentSeason, message: text });
      setError(text);
    } finally {
