...
import { trackNewsletterSubscribeError, trackNewsletterSubscribeSuccess, trackPopupClosed, trackPopupShown } from "../../lib/analytics/events";
...
const NewsletterPopup: React.FC = () => {
  ...
  const closePopup = useCallback(
    (suppressDays: number = 14, reason = "dismissed") => {
          ...
          >
            ...
            <div className="relative grid md:grid-cols-12">
              ...
              <div className="md:col-span-7 p-6 md:p-8">
                <h3 id="newsletter-popup-title" className="text-2xl md:text-3xl font-serif text-gray-900">Don’t you just like Ayo's photography skills?</h3>
                ...
                {!success ? (
                  <>
                    <form onSubmit={handleSubscribe} className="mt-6 flex flex-col sm:flex-row gap-3" aria-busy={submitting}>
                      ...
                      <button
                        type="submit"
                        disabled={submitting}
                        className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-accent-dark transition disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {submitting ? "Joining..." : "Join the List"}
                      </button>
                    </form>
                    {error && <div className="mt-2 text-sm text-red-600" role="alert" aria-live="assertive">{error}</div>}
                    {!error && submitting && <div className="mt-2 text-sm text-gray-500" role="status" aria-live="polite">Submitting...</div>}
