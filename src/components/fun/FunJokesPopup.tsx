...
import { trackJokeInteraction, trackPopupShown, trackPopupClosed } from "../../lib/analytics/events";
...
const FunJokesPopup: React.FC<FunJokesPopupProps> = ({ open, onClose }) => {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") close("escape_key");
    };
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="fun-jokes-popup-title"
          >
            {/* Close (top-right, red) */}
            <button
              ref={closeButtonRef}
              ...
              aria-label="Close"
              className="absolute top-3 right-3 w-9 h-9 rounded-full bg-red-600 text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-300 z-10"
            >
              ×
            </button>
            ...
            <div className="p-6 md:p-8">
              <div className="flex items-start justify-between">
                <div>
                  <h3 id="fun-jokes-popup-title" className="text-2xl md:text-3xl font-serif text-gray-900">A little photo humor</h3>
