...
import { dedupeByIdentity, isLocalEnvironment, toContactList, toSubscriberList, type AdminSubscriber } from "../../shared/helpers";
import type { Session } from "@supabase/supabase-js";
import { logAdminAction, logAdminError, logAdminWarning } from "../../../../lib/observability/logger";
import AdminAIInsightSection from "../../assistant/components/AdminAIInsightSection";
...
const AdminData: React.FC = () => {
  ...
  const getAIRecommendationLabel = (item: AdminAIInboxItem) => {
    if (item.draftStatus === "generated") return "Draft ready";
    if (item.recommendedCatalogItem?.label) return item.recommendedCatalogItem.label;
    if (item.recommendedAction === "custom_quote") return "Custom quote";
    return "Clarify";
  };
  ...
  const renderAIState = (contactId: string) => {
    ...
    if (item.analysisStatus === "succeeded") {
      return <p className="text-xs text-gray-600">{getAIRecommendationLabel(item)}</p>;
    }
    if (item.analysisStatus === "failed") {
      return <p className="text-xs text-rose-600">AI processing failed.</p>;
    }
    return <p className="text-xs text-gray-500">AI processing.</p>;
  };
                  ...
                  <table className="min-w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.12em] text-gray-500">Workflow</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.12em] text-gray-500">AI</th>
                      ...
                      {filteredContacts.map((c) => {
                        ...
                        return (
                          <tr key={c.id} className={`align-top transition-colors hover:bg-gray-50 ${isSelected ? "bg-blue-50/40" : ""}`}>
                            ...
                            <td className="px-6 py-4">
                              <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${serviceBadgeClass(c.service)}`}>{c.service}</span>
                              {c.service_tier && <p className="mt-2 text-xs text-gray-600">Tier: {c.service_tier}</p>}
                            </td>
                            <td className="px-6 py-4">
            ...
            <div className="sticky top-0 z-10 flex items-start justify-between border-b border-gray-200 bg-white px-4 py-3">
              <div>
                ...
                {selectedContact.service_tier ? (
                  <p className="mt-1 text-xs text-gray-500">Tier: {selectedContact.service_tier}</p>
                ) : null}
                {selectedWorkflow && (
